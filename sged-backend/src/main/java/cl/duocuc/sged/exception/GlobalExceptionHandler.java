 ...
 * Convierte excepciones en respuestas HTTP consistentes y claras.
 * 
 * Patrón: Mesero (Controller) → Chef (Service) → Error → Mesero captura
 */
@ControllerAdvice
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleJsonParseError(HttpMessageNotReadableException ex) {
        ErrorResponse error = ErrorResponse.builder()
                ...
                .status(HttpStatus.BAD_REQUEST.value())
                .error("JSON inválido")
                .message("El cuerpo de la petición no es un JSON válido: " + ex.getMostSpecificCause().getMessage())
                .build();
        return new ResponseEntity<>(error, HttpStatus.BAD_REQUEST);
    }
